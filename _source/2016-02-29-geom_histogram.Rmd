---
title: "geom_histogramをより使えるようになるために (ggplot2"
...
# ggplot2::geom_histogramを使いこなしたい
...
#### ラベル位置の調節  
`vjust`で鉛直 (vertical) 方向の位置を調節 (adjust) する  
```{r vosialize4}
InOutLab %>%
  ggplot(aes(x = value, fill = variable, group = variable)) + 
  geom_histogram(position = "identity", alpha = .5) +
...
## 補足
`stat`系は簡単な計算には便利だが速度は遅く、大規模データの可視化では先にready plotな状態にしてから`ggplot2`に渡すのがよいらしい   
```{r handling_for_hist}
InOutLab_wideForm <-
  Timecard %>%
  transmute(arrive = as.numeric(str_sub(Arrive, 1, 2)) + as.numeric(str_sub(Arrive, 4, 5)) / 60,
...
```
`stat_bin()`では、`geom_histogram()`で指定した`position = "identity`を引き継いでおらず、積み上げ型の表示になる  
`stat_bin()`内でも`position = "identity`を指定する必要がある  
