...
If all LHCII detached from PS II upon phosphorylation becomes attached instead to PS I [95,96], then these results fall short of the required increases in absorption cross-section of PS I of at least 16% for broad-band excitation and of 40-70%for Chi b excitation. (Allen 1992 BBA)

予想外の結果が出て、やや面白いけど、深く触れないとき。
~ cannot be distinguished and requires further investigation, which however is beyond the scope of the present study (Wargner et al. 2008, Planta)

The molecular mechanisms of primary redox-sensing steps are quite diverse and in some cases still not understood (Pfannschmidt et al. 2001)

・AはBと相関がある (A縦軸、B横軸)
A (show, represent) a (considerable, strong, positive correlation with B.
 cf) Dark respiration rate reportedly represents a positive correlation with the amount of leaf N per LA.
...
cf) leaf photosynthetic capacity of C3 plants is strongly dependent on the amount of N (Evans 1989)
・AはBによって説明できる (B⇒A)
A can be (largely, partly) accounted for by B.
 cf) The light-induced systemic regulation was largely accounted for by PPFD.
B might explain A.
A can be attributable to B.
A (possible, likely) cause of A is B 
